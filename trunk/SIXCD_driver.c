...
NTSTATUS SIXCDAddDevice(IN PDRIVER_OBJECT pDriverObject, IN PDEVICE_OBJECT pFdo)
{
	...
	KdPrint(("SIXCDAddDevice - Entry"));

	EnableSixaxis(pDevExt);
	...
	InitializeRemoveLock(&pDevExt->RemoveLock,'6ACD',0,0);
...
NTSTATUS SIXCDStartDevice(PDEVICE_OBJECT pFdo, PIRP pIrp)
{
	...
	if(!urb)
	{
		KdPrint(("SIXCDStartDevice - Unable to allocate memory for URB"));
	...
	if(!NT_SUCCESS(status))
	{
		ExFreePool(urb);
		ExFreePool(pcd);
		KdPrint(("SIXCDStartDevice - Error %X trying to read configuration descriptor 2", status));
		return status;
	}
	...
	{
		KdPrint(("SIXCDStartDevice - Could not create interrupt urb"));
	}
...
VOID SIXCDStopDevice(PDEVICE_OBJECT pFdo, PIRP pIrp)
{
	...
	if(pDevExt->DeviceStarted)
	{
		...
		if(!urb)
		{
			KdPrint(("SIXCDStopDevice - Could not allocate memory for URB"));
			return;
		}
...
*/
void SIXCDReadConfig(IN PDEVICE_OBJECT pFdo)
{
	HANDLE hKey;
	NTSTATUS ntstatus;
	int ReceivedDataLen;
	...
	*/
	...
	{
		KdPrint(("SIXCDReadConfig: IoOpenDeviceRegistryKey"));

		ReceivedDataLen = ReadRegistry(hKey, L"DevType", TempRegData, sizeof(TempRegData));
		if(ReceivedDataLen != 4)
		{
			KdPrint(("SIXCDReadConfig: DevType not found in registry. Using default."));
			pDevExt->DevUsage = HID_USAGE_GAMEPAD;
		}
		else
		{
		...
		*/
		...
		KdPrint(("SIXCDReadConfig: NrOfLayouts %i, LayoutNr %i",
		...
		for(index=0; index != pDevExt->NrOfLayouts; index++)
		{
			...
			{
				KdPrint(("SIXCDReadConfig: MapMatrix%i not found in registry. Using default layout.", index));
				setDefaultMapMatrix(pDevExt->MapMatrix[index], pDevExt->isWin9x);
			}

			/*
			Write retrieved data to the map matrix array used in SIXCD_control.
			...
			*/
			else
			{
				for(index2=0; index2 != MAP_MATRIX_SIZE; index2++)
				{
					...
					KdPrint(("SIXCDReadConfig: MapMatrix[%i]=%i", index2, pDevExt->MapMatrix[index][index2]));
				}
			}
		}
		...
		*/
		...
		if(ReceivedDataLen != 4)
		{
			KdPrint(("SIXCDReadConfig: ALFactor not found in registry. Using default."));
			pDevExt->LaFactor=255;
		}
		else
		...
		if(ReceivedDataLen != 4)
		{
			KdPrint(("SIXCDReadConfig: ARFactor not found in registry. Using default."));
			pDevExt->RaFactor=255;
		}
		else
		...
		if(ReceivedDataLen != 4)
		{
			KdPrint(("SIXCDReadConfig: AxesOn not found in registry. Using default."));
			if(pDevExt->isWin9x)
				pDevExt->AxesOn = 27; //no RY axis
			else
				pDevExt->AxesOn = 31;
		...
		if(ReceivedDataLen != 4)
		{
			KdPrint(("SIXCDReadConfig: BThreshold not found in registry. Using default."));
			pDevExt->BThreshold = MAX_VALUE * 10/255;
		}
		else
		{
		...
		if(ReceivedDataLen != 4)
		{
			KdPrint(("SIXCDReadConfig: TThreshold not found in registry. Using default."));
			pDevExt->TThreshold = MAX_VALUE * 10/255;
		}
		else
		{
		...
		if(ReceivedDataLen != 4)
		{
			KdPrint(("SIXCDReadConfig: AThreshold not found in registry. Using default."));
			pDevExt->AThreshold = MAX_VALUE * 100/255;
		}
		else
		{
		...
		if(ReceivedDataLen != 4)
		{
			KdPrint(("SIXCDReadConfig: LStickDZ not found in registry. Using default."));
			pDevExt->LStickDZ = 0;
		}
		else
		{
		...
		if(ReceivedDataLen != 4)
		{
			KdPrint(("SIXCDReadConfig: RStickDZ not found in registry. Using default."));
			pDevExt->RStickDZ = 0;
		}
		else
		{
		...
		{
			KdPrint(("SIXCDReadConfig: AxesScale not found in registry. Using default."));
			for(index=0; index<NR_OUT_AXES; index++)
			{
				pDevExt->AxesScale[index] = MAX_VALUE;
			}
		...
		if(ReceivedDataLen != 4)
		{
			KdPrint(("SIXCDReadConfig: LFullRange not found in registry. Using default."));
			pDevExt->bFullRange[0] = FALSE;
		}
		else
		{
		...
		if(ReceivedDataLen != 4)
		{
			KdPrint(("SIXCDReadConfig: RFullRange not found in registry. Using default."));
			pDevExt->bFullRange[1] = FALSE;
		}
		else
		{
		...
		if(ReceivedDataLen != 4)
		{
			KdPrint(("SIXCDReadConfig: NButtons not found in registry. Using default."));
			pDevExt->nButtons = NR_OUT_BUTTONS;
		}
		else
		{
